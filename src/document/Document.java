...
public abstract class Document {
	...
	{// how many parts did you break your word into ?
		...
		for (int i=0;i<word.length();i++) {
			...
			if(i==word.length()-1 && c=='e'&&flag&&counter>0)//if the word ends with 'e' and the one before it is constant
				counter--;
			if(c=='e'||c=='a'||c=='y'||c=='o'||c=='u'||c=='i') {
				
				if(flag) {
					flag=false;//as if conscuative vowels calculated as 1 syllables
					counter++;}
			}
			else flag=true;
		}
	...
	{
		double flesch=0;
		int wordNum=getNumWords();
		int sentenceNum = getNumSentences();
		int syllablesNum=getNumSyllables();
		flesch= 206.835 - ( 1.015 * (Double.valueOf(wordNum)/sentenceNum ) + (84.6 * (Double.valueOf(syllablesNum)/wordNum) ) );
		int x= 15,y=7;
		//System.out.println(" x/y : "+Double.valueOf(x)/7);
